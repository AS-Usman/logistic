...
export function RegistrationForm({ addToast, user, setUser, logoutUser }) {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      console.log('Flask backend offline, simulating account creation:', err);
      addToast(`Account created locally for ${username}! (Flask backend offline)`, 'success');
      setLoginData({ username: username, password: '' });
      setAuthMode('login');
    }
  };
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      console.log('Flask backend offline, simulating login:', err);
      if (setUser) {
        setUser({ username: loginData.username.trim(), user_id: 1 });
      }
      addToast(`Welcome back, ${loginData.username.trim()}! (Local session)`, 'success');
    }
  };
