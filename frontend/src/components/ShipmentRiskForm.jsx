...
export function ShipmentRiskForm({ onSaveShipment, addToast }) {
  ...
  const localRiskAnalysis = useDelayPredictor(formData);
  const [serverRiskAnalysis, setServerRiskAnalysis] = useState(null);
  const [predicting, setPredicting] = useState(false);
  const riskAnalysis = serverRiskAnalysis || localRiskAnalysis;
    ...
    try {
      const prediction = await apiRequest('/api/predict', {
        ...
        body: JSON.stringify({
          ...
          trafficCongestion: Number(formData.trafficCongestion),
          cargoWeight: Number(formData.cargoWeight)
        })
      });

      const serverAnalysis = prediction.data?.riskAnalysis;
      if (!serverAnalysis) throw new Error('Prediction response was incomplete.');
      setServerRiskAnalysis(serverAnalysis);
      ...
      const newShipment = {
        ...
        weatherCondition: formData.weatherCondition,
        route_risk: serverAnalysis.riskLevel || 'Low',
        delivery_status: 'In Transit',
        delay_days: Math.round((serverAnalysis.estimatedDelayHours || 0) / 24) || 0,
        transportMode: formData.transportMode,
        cargoCategory: formData.cargoCategory,
        cargoWeight: formData.cargoWeight,
        carrierRating: formData.carrierRating,
        trafficCongestion: formData.trafficCongestion,
        riskAnalysis: serverAnalysis,
        timestamp: new Date().toISOString()
      };

      const result = await onSaveShipment(newShipment);
      if (result?.ok) {
        addToast(`ML prediction successful: ${serverAnalysis.riskLevel} risk (${serverAnalysis.riskScore}%).`, 'success');
      }
    } catch (err) {
      console.error('Live ML prediction failed:', err);
      addToast(err.message || 'Prediction failed. Please check the Flask backend.', 'error');
    } finally {
      setPredicting(false);
    }
  };
      ...
      <div className="risk-predictor-wrapper">
        ...
        <div className="glass-card" style={{ padding: 36 }}>
          ...
          <form onSubmit={handleSave}>
            ...
            <button type="submit" className="btn" style={{ width: '100%' }} disabled={predicting}>
              <BookmarkPlus size={18} /> {predicting ? 'Running ML Prediction...' : 'Predict & Save Shipment'}
            </button>
          </form>
        </div>
        ...
        <div className="glass-card" style={{ padding: 36 }}>
          ...
          <div className="risk-meter-container">
            ...
            {serverRiskAnalysis && (
              <p style={{ textAlign: 'center', fontSize: '0.78rem', color: 'var(--text-subtle)', marginTop: 8 }}>
                Live Flask ML model • {serverRiskAnalysis.model} • confidence {Math.round((serverRiskAnalysis.confidence || 0) * 100)}%
              </p>
            )}
